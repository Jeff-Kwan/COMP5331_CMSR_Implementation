...
def load_param(path, weight_list):
    ...
    weight_dict = dict()
    if 'ffn' in weight_list:
        ffn_dict = {}
        # Load dense_1 weights and biases
        ffn_dict['layer1_weight'] = checkpoint['state_dict']['trm_encoder.layer.feed_forward.dense_1.weight'].detach().cpu()
        ffn_dict['layer1_bias'] = checkpoint['state_dict']['trm_encoder.layer.feed_forward.dense_1.bias'].detach().cpu()
        # Load dense_2 weights and biases
        ffn_dict['layer2_weight'] = checkpoint['state_dict']['trm_encoder.layer.feed_forward.dense_2.weight'].detach().cpu()
        ffn_dict['layer2_bias'] = checkpoint['state_dict']['trm_encoder.layer.feed_forward.dense_2.bias'].detach().cpu()
        weight_dict['ffn_dict'] = ffn_dict
    return weight_dict
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='de', type=str, help='the source market to pretrain, default is us')
