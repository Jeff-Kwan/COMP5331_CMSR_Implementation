...
def objective_function(config_dict=None, config_file_list=None, fixed_config_dict=None):
    ...
    config = Config(model=CATSR2, config_dict=config_dict, config_file_list=config_file_list)
    ...
    model = CATSR2(config, train_data.dataset, weight_dict).to(config['device'])
    logger.info(model)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_files', type=str, default='properties/CATSR.yaml properties/market.yaml',
                        help='config files list')
    parser.add_argument('--params_file', type=str, default='model.hyper', help='model hyper parameters')
    parser.add_argument('--output_file', type=str, default='hyper_example.result', help='output file')
    parser.add_argument('--weight_path', type=str, default='saved/CATSR-us-200.pth', help='pre-trained weight')
    parser.add_argument('--tool', type=str, default='Hyperopt', choices=['Hyperopt', 'Ray'], help='choose hypertuning tool')
