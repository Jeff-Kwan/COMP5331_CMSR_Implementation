...
def generate_raw_item_emb(df, emboutputpath):
    print("Storing raw text data instead of BERT embeddings...")

    # 获取原始文本特征
    raw_text_features = df["text_feature"].tolist()

    # 将原始文本数据存为embedding位置
    asin_list = df["asin"].tolist()
    temp = pd.DataFrame({"item_id:token": asin_list, "item_emb:token_seq": raw_text_features})

    # # 将文本特征转换为字符串格式保存
    # temp['item_emb:float_seq'] = temp['item_emb:float_seq'].apply(lambda x: ' '.join(x))

    # 保存为CSV文件
    temp.to_csv(emboutputpath, index=False)
    print(f"Done! Raw text data has been saved into: {emboutputpath}")
    return temp
...
def generate_item_emb(df, emboutputpath):
    ...
    device = set_device('0')
    tokenizer = AutoTokenizer.from_pretrained('/workspace/bert-base-uncased')
    model = AutoModel.from_pretrained('/workspace/bert-base-uncased').to(device)
...
if __name__ == "__main__":
    ...
    if use_bert:
        ...
        selected_meta_df["description"] = selected_meta_df.description.apply(list2str)
        selected_meta_df["text_feature"] = selected_meta_df[["title"]].agg(" ".join, axis=1)

        selected_meta_df = selected_meta_df.drop(columns=["title"])
        selected_meta_df = selected_meta_df.reset_index(drop=True)
        emb_df = generate_raw_item_emb(selected_meta_df, "item_emb.csv")
        emb_df = pd.read_csv("item_emb.csv")
    ...
    for m in market_list:
        ...
        df = df[['userId', 'itemId', 'timestamp', 'rate']].rename(columns={'userId': 'user_id:token', 'itemId': 'item_id:token', 'timestamp': 'timestamp:float', 'rate': 'rating:float'})
        df = df[['user_id:token', 'item_id:token', 'timestamp:float', 'rating:float']]
        if not os.path.exists(f'../dataset/{m}_raw'):
            os.mkdir(f'../dataset/{m}_raw')
        df.to_csv(f'../dataset/{m}_raw/{m}_raw.inter', sep='\t', index=False)
    
        m_emb_df = emb_df[emb_df['item_id:token'].isin(itemset)]
        m_emb_df.to_csv(f'../dataset/{m}_raw/{m}_raw.item', sep='\t', index=False)